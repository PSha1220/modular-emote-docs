...
- **オブジェクト名**  
  ここに値を入力すると、アバタービルド時にその名前へオブジェクト名を変更します。  
  空欄のままの場合は現在の GameObject 名をそのまま使用し、変更しません。  
...
- **ME FX マージを使用**  
  ...
  このオプションがオンのとき、関連する追加フィールドが表示されます。（**ME FX レイヤー**、**オブジェクト自動リネーム**、**オブジェクト名**）

- **オブジェクト自動リネーム**  
  設定したオブジェクト名を使って、アバタービルド時にオブジェクト名を自動で変更します。  
  マージされた ME FX と一緒に、ビルド後のオブジェクト名も揃えたい場合に使用します。
