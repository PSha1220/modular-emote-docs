...
- **ビルド用オブジェクト名**  
  ...
  **ME FX マージを使用** する場合は、後からオブジェクト名を変更してもアニメーションパスが変わって結果に影響しないよう、この値を設定して固定しておくことを推奨します。  
  ...
  この項目は詳細オプションの **ME FX マージを使用** を有効にしたときに利用できます。
...
- **ME FX マージを使用**  
  ...
  このオプションがオンのとき、関連する追加フィールドが表示されます。（**ME FX レイヤー**、**ビルド用オブジェクト名**）  
  ME FX 使用時に **ビルド用オブジェクト名** が空欄だと、後からオブジェクト名を変更した際にアニメーションパスが変わり、ビルド結果に影響する可能性があるため **情報メッセージ** が表示されます。  
  この場合、**Setup VRC Emote** を押すと現在の GameObject 名が **ビルド用オブジェクト名** に自動入力されます。
...
6. **ME FX マージを使用** が有効で **ビルド用オブジェクト名** が空欄の場合、現在の GameObject 名を自動入力
