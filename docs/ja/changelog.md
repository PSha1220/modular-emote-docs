...
- インスペクターが有効化されるたびに **開発者オプション** が強制的に折りたたまれる問題を修正しました。

### 改善

- 現在のエディターセッション中は **開発者オプション** の展開状態を維持するようにし、作業時の使いやすさを改善しました。
- 新規インポート時やエディターセッションの開始時は引き続き折りたたまれた状態になるため、通常のユーザーには整理されたインスペクターが表示されます。

## 1.5.1

### 変更

- **Build Object Name** 表記を、実際のコンポーネントオーバーレイ名に合わせて **ビルド時オブジェクト名** に変更しました。
- 関連するドキュメントと案内文の表記を **ビルド設定 / ビルド時オブジェクト名** 基準に更新しました。

### 追加

- Preview に **ラジアルプレビュー** を追加しました。
- ラジアル / リストプレビューを切り替えて、ビルド前に適用結果を確認できるようにしました。
