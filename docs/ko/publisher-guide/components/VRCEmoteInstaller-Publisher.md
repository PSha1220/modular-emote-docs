...
- **오브젝트 이름**  
  이 항목에 값을 입력하면 아바타 빌드 시 해당 이름으로 오브젝트 이름을 변경합니다.  
  공백으로 두면 기존 GameObject 이름을 그대로 따르며 변경하지 않습니다.  
  이 항목은 고급 옵션의 **병합 ME FX 사용**을 체크했을 때 활성화됩니다.
...
- **병합 ME FX 사용**  
  ...
  이 옵션이 켜져 있을 때, 관련 추가 필드가 노출됩니다. (**ME FX 레이어**, **오브젝트 이름 자동변환**, **오브젝트 이름**)

- **오브젝트 이름 자동변환**  
  설정한 오브젝트 이름을 기준으로 아바타 빌드 시 오브젝트 이름을 자동으로 변경합니다.  
  병합 ME FX를 사용할 때 빌드 결과의 오브젝트 이름을 함께 맞추고 싶을 경우 사용하는 옵션입니다.
