...
- **병합 ME FX 사용**  
  ...
  이 옵션이 켜져 있을 때, 관련 추가 필드가 노출됩니다. (**ME FX 레이어**, **빌드용 오브젝트 이름**)
  ME FX 사용 중 **빌드용 오브젝트 이름**이 비어 있으면 이후 오브젝트 이름 변경으로 인해 애니메이션 경로가 달라져 빌드 결과에 영향을 줄 수 있으므로 **안내 메시지**가 표시됩니다.
  이 경우 **Setup VRC Emote**를 누르면 현재 GameObject 이름이 **빌드용 오브젝트 이름**에 자동 입력됩니다.
...
6. **병합 ME FX 사용**이 켜져 있고 **빌드용 오브젝트 이름**이 비어 있으면 현재 GameObject 이름을 자동 입력
